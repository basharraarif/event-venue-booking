...
describe('DashboardPage', () => {
  ...
  test('shows loading state while auth is loading', () => {
    ...
    render(<DashboardPage />);
    expect(
      screen.getByText(/loading user information.../i)
    ).toBeInTheDocument();
  });
  ...
  test('fetches and displays "My Bookings" for any authenticated user', async () => {
    ...
    (bookingService.getMyBookings as jest.Mock).mockResolvedValue([
      {
        id: 'b1',
        event_details: { name: 'My Booked Event' },
        number_of_tickets: 2,
        total_price: '50.00',
        status: 'confirmed',
      },
  ...
  test('displays "My Events" section and fetches events for an Event Organizer', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/my events \(organized by me\)/i)
      ).toBeInTheDocument();
      expect(screen.getByText(/my organized event/i)).toBeInTheDocument();
    });
    expect(eventService.getEvents as jest.Mock).toHaveBeenCalledWith({
      organizer: mockUserOrganizer.id,
    });
  });
  ...
  test('does not display "My Events" section for a non-organizer (e.g. Customer)', async () => {
    ...
    await waitFor(() =>
      expect(bookingService.getMyBookings as jest.Mock).toHaveBeenCalled()
    );
    expect(
      screen.queryByText(/my events \(organized by me\)/i)
  ...
  test('displays "My Venues" section and fetches venues for a Venue Manager', async () => {
    ...
    (venueService.getVenues as jest.Mock).mockResolvedValue({
      results: [{ id: 'v1', name: 'My Managed Venue', capacity: 100 }],
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/my venues \(managed by me\)/i)
      ).toBeInTheDocument();
      expect(screen.getByText(/my managed venue/i)).toBeInTheDocument();
    });
    expect(venueService.getVenues as jest.Mock).toHaveBeenCalledWith({
      owner: mockUserVenueManager.id,
    });
  });
  ...
  test('does not display "My Venues" section for a non-venue manager (e.g. Customer)', async () => {
    ...
    expect(
      screen.queryByText(/my venues \(managed by me\)/i)
    ).not.toBeInTheDocument();
  ...
  test('displays all relevant sections for a user with multiple roles', async () => {
    setupMockAuth(mockUserAllRoles, [
      ROLE_CUSTOMER,
    ...
    (bookingService.getMyBookings as jest.Mock).mockResolvedValue([
      {
        id: 'b1',
        event_details: { name: 'Multi-role Booking' },
        number_of_tickets: 1,
        total_price: '20.00',
        status: 'pending',
      },
    ...
    (eventService.getEvents as jest.Mock).mockResolvedValue([
      {
        id: 'e1',
        name: 'Multi-role Event',
        start_time: new Date().toISOString(),
        status: 'upcoming',
      },
    ]);
    (venueService.getVenues as jest.Mock).mockResolvedValue({
      results: [{ id: 'v1', name: 'Multi-role Venue', capacity: 200 }],
    });
    ...
    expect(bookingService.getMyBookings as jest.Mock).toHaveBeenCalledTimes(1);
    expect(eventService.getEvents as jest.Mock).toHaveBeenCalledWith({
      organizer: mockUserAllRoles.id,
    });
    expect(venueService.getVenues as jest.Mock).toHaveBeenCalledWith({
      owner: mockUserAllRoles.id,
    });
  });
  ...
  test('shows error message for bookings if fetching fails', async () => {
    ...
    (bookingService.getMyBookings as jest.Mock).mockRejectedValueOnce(
      new Error('Failed to load bookings')
    );
    render(<DashboardPage />);
    await waitFor(() => {
      expect(
        screen.getByText(/failed to load your bookings/i)
      ).toBeInTheDocument();
    });
  });
});
