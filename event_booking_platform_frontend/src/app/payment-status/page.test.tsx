...
jest.mock('@stripe/react-stripe-js', () => ({
  ...
  Elements: ({ children }: { children: React.ReactNode }) => (
    <div data-testid="stripe-elements-wrapper">{children}</div>
  ),
}));
...
describe('PaymentStatusPageWrapper (including PaymentStatusPage)', () => {
  ...
  it('shows error if essential URL parameters are missing', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('alert-message')).toHaveTextContent(
        'Required payment information is missing from the URL.'
      );
      expect(screen.getByTestId('alert-message')).toHaveAttribute(
        'data-type',
        'error'
      );
    });
  ...
  it('handles failed payment (requires_payment_method) via Stripe.js retrievePaymentIntent', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('alert-message')).toHaveTextContent(
        'Payment requires further action or failed. Please try again or contact support.'
      );
      expect(screen.getByTestId('alert-message')).toHaveAttribute(
        'data-type',
        'error'
  ...
  it('handles successful payment via backend getPaymentDetails fallback', async () => {
    ...
    await waitFor(() => {
      expect(mockPaymentService.getPaymentDetails).toHaveBeenCalledWith(
        'local_pay_backend_succ'
      );
      expect(screen.getByTestId('alert-message')).toHaveTextContent(
        'Payment successful! Your booking is confirmed.'
      );
  ...
  it('handles error when Stripe.js retrievePaymentIntent fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('alert-message')).toHaveTextContent(
        'Error verifying payment status with Stripe.'
      );
    });
  });
  ...
  it('handles error when backend getPaymentDetails fails', async () => {
    ...
    mockPaymentService.getPaymentDetails.mockRejectedValue(
      new Error('Backend API Error')
    );
    ...
    await waitFor(() => {
      expect(screen.getByTestId('alert-message')).toHaveTextContent(
        'Could not retrieve payment status from our records.'
      );
    });
  });
});
