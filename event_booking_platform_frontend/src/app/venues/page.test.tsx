...
describe('VenuesPage Component', () => {
  ...
  it('fetches and displays venues successfully', async () => {
    mockGetVenues.mockResolvedValueOnce({
      results: mockVenuesData,
      count: mockVenuesData.length,
      next: null,
      previous: null,
    });
    renderWithAuthProvider(<VenuesPage />);
  ...
  it('displays "No venues found" when no venues are returned', async () => {
    ...
    expect(
      screen.getByText(/no venues found matching your criteria./i)
  ...
  describe('Filtering Functionality', () => {
    ...
    it('fetches venues with correct parameters when "Apply Filters" is clicked', async () => {
      ...
      fireEvent.change(screen.getByLabelText(/min. capacity/i), {
        target: { value: '50' },
      });
      fireEvent.change(screen.getByLabelText(/availability/i), {
        target: { value: 'true' },
      });
      fireEvent.change(screen.getByLabelText(/min. price\/hour/i), {
        target: { value: '40' },
      });
      fireEvent.change(screen.getByLabelText(/max. price\/hour/i), {
        target: { value: '60' },
      });
    ...
    it('clears filters and fetches all venues when "Clear Filters" is clicked', async () => {
      ...
      await waitFor(() =>
        expect(screen.getByText('Filtered Alpha Only')).toBeInTheDocument()
      );
      expect(mockGetVenues).toHaveBeenLastCalledWith(
      ...
      mockGetVenues.mockResolvedValueOnce({
        results: refreshedMockVenues,
        count: refreshedMockVenues.length,
        next: null,
        previous: null,
      });
      ...
      expect(
        (screen.getByLabelText(/min. capacity/i) as HTMLInputElement).value
      ).toBe('');
      expect(
        (screen.getByLabelText(/availability/i) as HTMLSelectElement).value
      ).toBe('');
      expect(
        (screen.getByLabelText(/min. price\/hour/i) as HTMLInputElement).value
      ).toBe('');
      expect(
        (screen.getByLabelText(/max. price\/hour/i) as HTMLInputElement).value
      ).toBe('');
      ...
      await waitFor(() => {
        ...
        expect(
          screen.queryByText('Filtered Alpha Only')
