...
describe('EditVenuePage Component (with Role and Ownership Protection)', () => {
  ...
  it('redirects unauthenticated user', async () => {
    ...
    expect(
      screen.queryByRole('heading', { name: /edit venue/i })
  ...
  it('fetches venue data and pre-fills the form for VENUE_MANAGER owner', async () => {
    ...
    expect(screen.getByLabelText(/capacity/i)).toHaveValue(
      mockVenueData.capacity
    );
    // Ensure amenities are joined correctly if they are an array
    const amenitiesValue = Array.isArray(mockVenueData.amenities)
      ? mockVenueData.amenities.join(', ')
      : mockVenueData.amenities;
  ...
  it('calls updateVenue and redirects on successful submission for VENUE_MANAGER owner', async () => {
    ...
    mockUpdateVenue.mockResolvedValueOnce({
      ...updatedVenueDetails,
      id: 1,
      created_at: '',
      updated_at: '',
    ...
    renderWithAuthProvider(<EditVenuePageInternal />);
    await waitFor(() =>
      expect(screen.getByLabelText(/name/i)).toHaveValue(mockVenueData.name)
    ); // Ensure form is pre-filled

    fireEvent.change(screen.getByLabelText(/name/i), {
      target: { value: updatedVenueDetails.name },
    });
    fireEvent.change(screen.getByLabelText(/address/i), {
      target: { value: updatedVenueDetails.address },
    });
    fireEvent.change(screen.getByLabelText(/capacity/i), {
      target: { value: String(updatedVenueDetails.capacity) },
    });
    fireEvent.change(screen.getByLabelText(/amenities/i), {
      target: { value: (updatedVenueDetails.amenities as string[]).join(', ') },
    });
    fireEvent.change(screen.getByLabelText(/price per hour/i), {
      target: { value: updatedVenueDetails.pricing_per_hour },
    });
    fireEvent.change(screen.getByLabelText(/price per day/i), {
      target: { value: '' },
    }); // Explicitly clear the field for null
    ...
    expect(
      screen.getByText(/venue "Updated Venue Name" updated successfully!/i)
    ).toBeInTheDocument();
    await waitFor(
      () => {
  ...
  it('displays an error message if getVenueById fails', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/failed to fetch venue details: Venue fetch failed/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('displays an error message if updateVenue fails', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/name/i), {
      target: { value: 'Attempt Update' },
    });
    fireEvent.submit(screen.getByRole('button', { name: /update venue/i }));

    await waitFor(() => {
      expect(
        screen.getByText(/failed to update venue: Update Failed/i)
      ).toBeInTheDocument();
    });
