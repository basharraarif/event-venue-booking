...
const renderPage = (authContextValue: Partial<ReturnType<typeof useAuth>>, venueIdParam = 'venue1') => {
...
describe('VenueDetailPage', () => {
  const venueManagerOwner = { id: "owner123", username: "venueOwner", roles: [ROLE_VENUE_MANAGER] };
  const venueManagerNonOwner = { id: "otherVM", username: "otherVM", roles: [ROLE_VENUE_MANAGER] };
  const customerUser = { id: "customer1", username: "customer", roles: [ROLE_CUSTOMER] };
  ...
  it('renders loading state initially', () => {
    ...
    expect(screen.getByTestId('loading-spinner')).toHaveTextContent('Loading venue details...');
  });
  ...
  it('shows error if venueId is missing (though useParams mock usually provides it)', () => {
    ...
    expect(screen.getByTestId('alert-message')).toHaveTextContent('Venue ID is missing.');
  });

  it('fetches and displays venue details', async () => {
    renderPage({ isAuthenticated: true, user: customerUser as any, hasRole: (r:string) => customerUser.roles.includes(r) });
    await waitFor(() => expect(mockGetVenueById).toHaveBeenCalledWith('venue1'));
    expect(screen.getByRole('heading', { name: mockVenueData.name })).toBeInTheDocument();
    expect(screen.getByText(mockVenueData.address)).toBeInTheDocument();
    expect(screen.getByText(`Capacity: ${mockVenueData.capacity}`)).toBeInTheDocument();
  });

  it('shows error if fetching venue details fails', async () => {
    mockGetVenueById.mockRejectedValueOnce(new Error('Failed to fetch'));
    renderPage({ isAuthenticated: true, user: customerUser as any, hasRole: (r:string) => customerUser.roles.includes(r) });
    await waitFor(() => expect(screen.getByTestId('alert-message')).toHaveTextContent('Failed to load venue details.'));
  });
  ...
  describe('Edit and Delete Buttons Visibility', () => {
    it('shows Edit and Delete buttons for Venue Manager who owns the venue', async () => {
      ...
      await waitFor(() => expect(screen.getByRole('heading', { name: mockVenueData.name })).toBeInTheDocument()); // Wait for venue data
      expect(screen.getByRole('link', { name: /edit venue/i })).toBeInTheDocument();
    ...
    it('does not show Edit and Delete buttons for Venue Manager who does not own the venue', async () => {
      ...
      await waitFor(() => expect(screen.getByRole('heading', { name: mockVenueData.name })).toBeInTheDocument());
      expect(screen.queryByRole('link', { name: /edit venue/i })).not.toBeInTheDocument();
  ...
  describe('Delete Venue Functionality', () => {
    ...
    it('calls deleteVenue service and redirects on successful deletion by owner', async () => {
      ...
      await waitFor(() => expect(screen.getByRole('button', { name: /delete venue/i })).toBeInTheDocument());

      fireEvent.click(screen.getByRole('button', { name: /delete venue/i }));

      expect(window.confirm).toHaveBeenCalledWith(`Are you sure you want to delete venue "${mockVenueData.name}"? This action cannot be undone.`);

      await waitFor(() => expect(mockDeleteVenue).toHaveBeenCalledWith(mockVenueData.id));
      ...
      await waitFor(() => expect(mockRouterPush).toHaveBeenCalledWith('/venues'));
    });
    ...
    it('shows error message if deleteVenue service fails', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('alert-message')).toHaveTextContent(/Failed to delete venue: Deletion failed/i);
      });
