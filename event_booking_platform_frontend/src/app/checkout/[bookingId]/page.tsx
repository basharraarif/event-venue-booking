...
interface FullBookingDetails {
  id: string;
  event_details?: {
    // Assuming event_details is nested from your BookingSerializer
    name: string;
    start_time: string;
...
const CheckoutPage = () => {
  ...
  const [bookingDetails, setBookingDetails] =
    useState<FullBookingDetails | null>(null);
    ...
    const initializeCheckout = async () => {
      ...
      try {
        ...
        if (fetchedBooking.status !== 'pending_payment') {
          // Changed 'pending' to 'pending_payment'
          setError(
            `This booking is already ${fetchedBooking.status} and cannot be paid for. Payment can only be made for 'pending_payment' bookings.`
          );
          setLoading(false);
          return;
        }
        ...
        if (
          ...
          paymentIntentResponse.payment_id
        ) {
        ...
        setError(
          err.response?.data?.detail ||
            err.message ||
            'An unexpected error occurred during checkout initialization.'
        );
      } finally {
        setLoading(false);
      }
  ...
  if (authLoading || (loading && !bookingDetails && !error)) {
    // Show main loader if auth or initial data is loading
  ...
  if (error) {
    return (
      <div className="container mx-auto p-4 text-center">
        ...
        <Link href="/dashboard" className="link-primary mt-4 inline-block">
          Go to Dashboard
        </Link>
      </div>
    );
  ...
  if (!clientSecret || !options || !paymentId) {
    return (
      <div className="container mx-auto p-4 text-center">
        <AlertMessage
          message="Could not initialize payment gateway. Required information is missing."
          type="error"
        />
        <Link href="/dashboard" className="link-primary mt-4 inline-block">
          Go to Dashboard
      ...
      <div className="mb-6 p-6 bg-white dark:bg-gray-800 shadow-md rounded-lg">
        ...
        <div className="space-y-2">
          <p>
            ...
            {bookingDetails.event_details?.name || 'N/A'}
          </p>
          <p>
            <span className="font-semibold">Tickets:</span>{' '}
            {bookingDetails.number_of_tickets}
          </p>
          {bookingDetails.price_per_ticket_at_booking && ( // Conditional rendering
            <p>
              <span className="font-semibold">Price per Ticket:</span> $
              {parseFloat(bookingDetails.price_per_ticket_at_booking).toFixed(
                2
              )}
            </p>
          )}
