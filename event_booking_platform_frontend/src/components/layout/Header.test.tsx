...
describe('Header Component', () => {
  ...
  it('renders links for authenticated customer user', () => {
    ...
    mockHasRole = jest.fn((roleName: string) =>
      customerUser.roles.includes(roleName)
    );
    (useAuth as jest.Mock).mockReturnValueOnce({
    ...
    expect(
      screen.getByText(`Logout (${customerUser.username})`)
    ).toBeInTheDocument();
  ...
  it('shows "Create Event" link for Event Organizer', () => {
    ...
    mockHasRole = jest.fn((roleName: string) =>
      eventOrganizerUser.roles.includes(roleName)
    );
    (useAuth as jest.Mock).mockReturnValueOnce({
      isAuthenticated: true,
  ...
  it('shows "Create Venue" link for Venue Manager', () => {
    ...
    mockHasRole = jest.fn((roleName: string) =>
      venueManagerUser.roles.includes(roleName)
    );
    (useAuth as jest.Mock).mockReturnValueOnce({
      isAuthenticated: true,
  ...
  it('shows both "Create Event" and "Create Venue" links for user with both roles', () => {
    const multiRoleUser = {
      ...
      roles: [ROLE_EVENT_ORGANIZER, ROLE_VENUE_MANAGER],
    };
    mockHasRole = jest.fn((roleName: string) =>
      multiRoleUser.roles.includes(roleName)
    );
    (useAuth as jest.Mock).mockReturnValueOnce({
      isAuthenticated: true,
      user: multiRoleUser,
