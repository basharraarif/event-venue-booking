...
describe('VenueForm Component', () => {
  ...
  it('renders form with initial data', () => {
    ...
    expect(screen.getByLabelText(/capacity/i)).toHaveValue(
      mockInitialData.capacity
    );
    expect(screen.getByLabelText(/amenities/i)).toHaveValue(
      (mockInitialData.amenities as string[]).join(', ')
    );
    expect(screen.getByLabelText(/price per hour/i)).toHaveValue(
      parseFloat(mockInitialData.pricing_per_hour!)
    );
    expect(screen.getByLabelText(/price per day/i)).toHaveValue(
      parseFloat(mockInitialData.pricing_per_day!)
    );
    expect(screen.getByLabelText(/is available/i)).toBeChecked();
  });
  ...
  it('allows user to fill the form', () => {
    ...
    expect(screen.getByLabelText(/price per hour/i)).toHaveValue(75.5);
  ...
  it('displays custom submit button text', () => {
    render(
      <VenueForm onSubmit={mockOnSubmit} submitButtonText="Update Venue" />
    );
    expect(
  ...
  it('correctly processes amenities string for submission', async () => {
    ...
    const testCases = [
      {
        input: 'WiFi, Projector, Stage',
        expected: ['WiFi', 'Projector', 'Stage'],
      },
      {
        input: '  WiFi ,  Projector  ,Stage  ',
        expected: ['WiFi', 'Projector', 'Stage'],
    ...
    for (const tc of testCases) {
      ...
      fireEvent.change(screen.getByLabelText(/name/i), {
        target: { value: 'Amenity Test Venue' },
      });
      fireEvent.change(screen.getByLabelText(/address/i), {
        target: { value: '123 Amenity St' },
      });
      fireEvent.change(screen.getByLabelText(/capacity/i), {
        target: { value: '100' },
      });
