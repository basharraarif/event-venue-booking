...
const VenueCard: React.FC<VenueCardProps> = ({ venue }) => {
  return (
    ...
    <div
      className="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden m-0 md:m-4"
      data-cy="venue-card"
    >
      ...
      <div className="p-4 md:p-6">
        ...
        {/* Added height and overflow for consistency */}
        <div className="mb-4 flex flex-wrap gap-2">
          <span
            className="inline-block bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300 text-xs font-semibold px-2.5 py-0.5 rounded-full"
            data-cy="venue-capacity"
          >
            Capacity: {venue.capacity}
          </span>
          {venue.is_available ? (
            <span
              className="inline-block bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300 text-xs font-semibold px-2.5 py-0.5 rounded-full"
              data-cy="venue-availability-status"
            >
              Available
            </span>
          ) : (
            <span
              className="inline-block bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300 text-xs font-semibold px-2.5 py-0.5 rounded-full"
              data-cy="venue-availability-status"
            >
              Not Available
            </span>
        ...
        <div className="text-gray-700 dark:text-gray-300 mb-1 text-sm">
          {venue.pricing_per_hour && (
            <p data-cy="venue-price-per-hour">
              <span className="font-semibold">Hourly:</span> $
              {parseFloat(venue.pricing_per_hour).toFixed(2)}
            </p>
          )}
          {venue.pricing_per_day && (
            <p>
              <span className="font-semibold">Daily:</span> $
              {parseFloat(venue.pricing_per_day).toFixed(2)}
            </p>
          )}
          ...
          <div className="mt-3">
            ...
            <div className="flex flex-wrap gap-1">
              {venue.amenities.slice(0, 3).map(
                (
                  amenity,
                  index // Show first 3 amenities
                ) => (
                  <span
                    key={index}
                    className="text-xs bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-2 py-0.5 rounded-full"
                  >
