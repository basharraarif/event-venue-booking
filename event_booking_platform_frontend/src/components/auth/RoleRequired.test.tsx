...
describe('RoleRequired Component', () => {
  ...
  it('shows loading spinner when auth context is loading', () => {
    ...
    render(
      <RoleRequired requiredRoles="ANY_ROLE">
        <TestChildComponent />
      </RoleRequired>
    );
  ...
  it('redirects to fallbackUrl if user is not authenticated', async () => {
    ...
    render(
      <RoleRequired requiredRoles="ANY_ROLE" fallbackUrl="/login">
        <TestChildComponent />
      </RoleRequired>
    );
  ...
  it('renders children if user has the required single role', () => {
    ...
    render(
      <RoleRequired requiredRoles="USER">
        <TestChildComponent />
      </RoleRequired>
    );
  ...
  it('shows error message if showError is true and user does not have the role', () => {
    ...
    mockHasRole = jest.fn((role: string) =>
      userWithoutRole.roles.includes(role)
    );
    (useAuth as jest.Mock).mockReturnValueOnce({
      isLoading: false,
      isAuthenticated: true,
    ...
    render(
      <RoleRequired requiredRoles="ADMIN" showError={true}>
        <TestChildComponent />
      </RoleRequired>
    );
