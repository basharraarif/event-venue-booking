  ...
  it('should display venue cards if venues exist, or a "no venues" message', () => {
    cy.get('body').then(($body) => {
      if ($body.find('[data-cy="venue-card"]').length > 0) {
        ...
        cy.get('[data-cy="loading-venues-message"]', { timeout: 10000 }).should(
          'not.exist'
        );
        ...
        cy.log(
          'Neither venue cards nor "no venues" message found. Error page?'
        );
...
describe('Venue Listing - Filtering and Searching', () => {
  ...
  const mockVenuesSeed = [
    {
      ...
      capacity: 200,
      is_available: true,
      pricing_per_hour: '150.00',
      description: 'Opulent hall for grand events.',
      amenities: { wifi: true, stage: true },
    },
    {
      id: 2,
    ...
    },
    {
      ...
      capacity: 500,
      is_available: true,
      pricing_per_hour: '300.00',
      description: 'State-of-the-art facility.',
      amenities: { wifi: true, parking: true, catering: true },
    },
    {
      id: 4,
      name: 'Riverside Pavilion',
      address: '101 River Bend',
      capacity: 100,
      is_available: true,
      pricing_per_hour: '80.00',
      description: 'Scenic outdoor setting.',
      amenities: { outdoor_space: true },
    },
    {
      id: 5,
      name: 'Downtown Studio Loft',
      address: '55 Central Sq',
      capacity: 75,
      is_available: true,
      pricing_per_hour: '120.00',
      description: 'Chic urban space.',
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', '/api/venues*', (req) => {
      ...
      if (!req.url.includes('?')) {
        // A very basic check for no query params
        req.reply({
          statusCode: 200,
          body: {
            results: mockVenuesSeed,
  ...
  it('should filter venues by search term (name)', () => {
    ...
    const expectedResult = mockVenuesSeed.filter((v) =>
      v.name.includes('Grand Plaza')
    );
    cy.intercept(
      'GET',
      `/api/venues*search=${encodeURIComponent(searchTerm)}*`,
      {
        statusCode: 200,
        body: {
    ...
    cy.get('[data-cy="venue-card"]').should(
      'have.length',
    ...
    if (expectedResult.length > 0) {
      cy.get('[data-cy="venue-card"]')
        .first()
        .find('[data-cy="venue-name"]')
        .should('contain.text', 'Grand Plaza Hall');
    }
  });

  it('should filter venues by minimum capacity', () => {
    const minCapacity = 150;
    const expectedResult = mockVenuesSeed.filter(
      (v) => v.capacity >= minCapacity
    );
    ...
    cy.get('[data-cy="filter-capacity-input"]')
      .clear()
      .type(minCapacity.toString());
  ...
  it('should filter venues by availability (Available)', () => {
    ...
    cy.get('[data-cy="venue-card"]').each(($card) => {
      cy.wrap($card)
        .find('[data-cy="venue-availability-status"]')
  ...
  it('should filter venues by price range (e.g., $70 to $160 per hour)', () => {
    ...
    cy.intercept(
      'GET',
      `/api/venues*pricing_per_hour__gte=${minPrice}*pricing_per_hour__lte=${maxPrice}*`,
      {
        statusCode: 200,
        body: {
          results: expectedResult,
    ...
    cy.get('[data-cy="filter-min-price-input"]')
      .clear()
    ...
    cy.get('[data-cy="filter-max-price-input"]')
      .clear()
    ...
    cy.get('[data-cy="venue-card"]').each(($card) => {
      cy.wrap($card)
        .find('[data-cy="venue-price-per-hour"]')
        .invoke('text')
        .then((text) => {
  ...
  it('should paginate to the next page', () => {
    ...
    cy.intercept('GET', '/api/venues*', initialResponse).as(
      'getInitialPageForPagination'
    );
    cy.visit('/venues');
    ...
    cy.get('[data-cy="pagination-page-display"]').should(
      'contain.text',
      'Page 2'
    );
    ...
    cy.get('[data-cy="venue-card"]')
      .first()
      .find('[data-cy="venue-name"]')
      .should('contain.text', mockVenuesSeed[2].name);
  ...
  it('should update results when a filter is cleared by user', () => {
    const searchTerm = 'Grand Plaza';
    const expectedResultSearch = mockVenuesSeed.filter((v) =>
    ...
    cy.intercept(
      ...
      {
        body: {
          results: expectedResultSearch,
          count: expectedResultSearch.length,
        },
      }
    ).as('getSearch');
    ...
    cy.intercept('GET', '/api/venues*', (req) => {
      if (!req.url.includes('search=') || req.url.includes('search=&')) {
        // Simplistic check for empty search
        req.reply({
          body: { results: mockVenuesSeed, count: mockVenuesSeed.length },
        });
      }
