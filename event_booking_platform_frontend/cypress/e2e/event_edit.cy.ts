...
describe('Event Edit Page (/dashboard/organizer/events/edit/:eventId)', () => {
    ...
    const mockEventData = {
        ...
        start_time: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toISOString().substring(0, 16),
        end_time: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000).toISOString().substring(0, 16),
        ticket_price: '30.00',
        max_capacity: 150,
        status: 'upcoming',
    ...
    before(() => {
        ...
        cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/registration/`, body: { username: adminUser.username, email: adminUser.email, password: adminUser.password, password2: adminUser.password }, failOnStatusCode: false })
            .then(() => cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: adminUser.email, password: adminUser.password } }))
            .then(loginResp => {
                adminToken = loginResp.body.key;
                cy.request({ method: 'GET', url: `${Cypress.env('apiUrl')}/auth/user/`, headers: { Authorization: `Token ${adminToken}` }})
                .then(userResp => {
                    ...
                    cy.request({
                        ...
                        body: { name: `Test Venue for Event Edit ${uniqueId}`, address: '123 Edit St', capacity: 100 }
                    }).then(venueResp => {
                        ...
                        cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/registration/`, body: { username: organizer1User.username, email: organizer1User.email, password: organizer1User.password, password2: organizer1User.password } })
                            .then(() => cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: organizer1User.email, password: organizer1User.password } }))
                            .then(org1LoginResp => {
                                organizer1Token = org1LoginResp.body.key;
                                cy.wrap(organizer1Token).as('organizer1Token');
                                // Get organizer1's actual ID
                                cy.request({ method: 'GET', url: `${Cypress.env('apiUrl')}/auth/user/`, headers: { Authorization: `Token ${organizer1Token}` }})
                                .then(org1UserResp => {
                                    ...
                                    cy.request({
                                        ...
                                        body: { ...mockEventData, categories: mockEventData.categories.map(c => c.name) } // Send category names for creation
                                    }).then(eventResp => {
        ...
        cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/registration/`, body: { username: organizer2User.username, email: organizer2User.email, password: organizer2User.password, password2: organizer2User.password } })
            .then(() => cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: organizer2User.email, password: organizer2User.password } }))
            .then(loginResp => { organizer2Token = loginResp.body.key; cy.wrap(organizer2Token).as('organizer2Token'); });

        // Create Customer User
        cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/registration/`, body: { username: customerUser.username, email: customerUser.email, password: customerUser.password, password2: customerUser.password } })
            .then(() => cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: customerUser.email, password: customerUser.password } }))
            .then(loginResp => { customerToken = loginResp.body.key; cy.wrap(customerToken).as('customerToken'); });
    });

    beforeEach(function() { // Use function() to access alias context
    ...
    it('shows access denied for Customer user', function() {
        ...
        cy.get('h1').contains(`Edit Event: ${mockEventData.name}`).should('not.exist');
    });
    ...
    it('shows access denied for Event Organizer who is not the owner', function() {
        ...
        cy.intercept('GET', `${Cypress.env('apiUrl')}/auth/user/`, { statusCode: 200, body: { id: 'org2UserId', roles: [{ name: ROLE_EVENT_ORGANIZER }] } }).as('getNonOwnerEO');
        cy.intercept('GET', `${Cypress.env('apiUrl')}/events-management/events/${this.eventByOrganizer1Id}/`, { statusCode: 200, body: { ...mockEventData, id: this.eventByOrganizer1Id, organizer: mockEventData.organizer } }).as('getEventToEdit');

        cy.visit(`/dashboard/organizer/events/edit/${this.eventByOrganizer1Id}`);
        cy.wait(['@getNonOwnerEO', '@getEventToEdit']);

        cy.contains('You are not authorized to edit this event.').should('be.visible');
        cy.get('h1').contains(`Edit Event: ${mockEventData.name}`).should('not.exist');
    });

    context('When logged in as Event Organizer (Owner)', function() {
        beforeEach(function() {
            if (!this.eventByOrganizer1Id || !this.organizer1Token || !mockEventData.organizer) this.skip();
            localStorage.setItem('authToken', this.organizer1Token);
            cy.intercept('GET', `${Cypress.env('apiUrl')}/auth/user/`, { statusCode: 200, body: { id: mockEventData.organizer, roles: [{ name: ROLE_EVENT_ORGANIZER }] } }).as('getOwnerEO');
            cy.intercept('GET', `${Cypress.env('apiUrl')}/events-management/events/${this.eventByOrganizer1Id}/`, { statusCode: 200, body: { ...mockEventData, id: this.eventByOrganizer1Id, organizer: mockEventData.organizer, venue: testVenueId, categories: mockEventData.categories.map(c => c.name) } }).as('getEventToEdit');
            // Mock calls for EventForm data if it makes them (e.g., categories, venues)
            cy.intercept('GET', `${Cypress.env('apiUrl')}/venues/?limit=1000&offset=0`, { statusCode: 200, body: { results: [{id: testVenueId, name: 'Test Venue for Event Edit'}]} }).as('getVenuesForForm');
            cy.intercept('GET', `${Cypress.env('apiUrl')}/events-management/categories/`, { statusCode: 200, body: mockEventData.categories }).as('getCategoriesForForm');

            cy.visit(`/dashboard/organizer/events/edit/${this.eventByOrganizer1Id}`);
            cy.wait(['@getOwnerEO', '@getEventToEdit', '@getVenuesForForm', '@getCategoriesForForm']);
        });

        it('allows owner EVENT_ORGANIZER to see the edit page with pre-filled form', function() {
            cy.url().should('include', `/dashboard/organizer/events/edit/${this.eventByOrganizer1Id}`);
            cy.get('h1').contains(`Edit Event: ${mockEventData.name}`).should('be.visible');
        ...
        it('allows owner EVENT_ORGANIZER to update the event (placeholder test)', function() {
            ...
            cy.intercept('PATCH', `${Cypress.env('apiUrl')}/events-management/events/${this.eventByOrganizer1Id}/`).as('updateEventApi');
            ...
            cy.log('Placeholder test for event update by owner. Actual form interaction depends on EventForm component.');
