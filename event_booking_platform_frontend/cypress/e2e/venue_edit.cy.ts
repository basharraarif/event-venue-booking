...
describe('Venue Edit Page (/venues/:id/edit)', () => {
    ...
    before(() => {
        // Create users
        cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/registration/`, body: { username: venueManagerOwner.username, email: venueManagerOwner.email, password: venueManagerOwner.password, password2: venueManagerOwner.password } })
            .then(() => cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: venueManagerOwner.email, password: venueManagerOwner.password } }))
            .then(loginResp => {
                ...
                cy.request({ method: 'GET', url: `${Cypress.env('apiUrl')}/auth/user/`, headers: { Authorization: `Token ${venueManagerOwnerToken}` }})
                  .then(userResp => {
                    ...
                    cy.request({
                        ...
                        body: { name: mockVenueData.name, address: mockVenueData.address, capacity: mockVenueData.capacity, amenities: mockVenueData.amenities, pricing_per_hour: mockVenueData.pricing_per_hour }
                    }).then(venueResp => {
        ...
        cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/registration/`, body: { username: venueManagerNonOwner.username, email: venueManagerNonOwner.email, password: venueManagerNonOwner.password, password2: venueManagerNonOwner.password } })
            .then(() => cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: venueManagerNonOwner.email, password: venueManagerNonOwner.password } }))
            .then(loginResp => {
    ...
    beforeEach(function() { // Use function() to access alias context
        ...
        if (this.ownedVenueId) {
            cy.intercept('GET', `${Cypress.env('apiUrl')}/venues/${this.ownedVenueId}/`, {
                statusCode: 200,
                body: { ...mockVenueData, id: this.ownedVenueId, owner: mockVenueData.owner.id }, // Return owner ID
            }).as('getVenueDetails');
        }
    });
    ...
    it('shows access denied for Customer user', function() {
        ...
        cy.get('h1').contains(`Edit Venue: ${mockVenueData.name}`).should('not.exist');
    });
    ...
    it('shows access denied for Venue Manager who is not the owner', function() {
        ...
        cy.intercept('GET', `${Cypress.env('apiUrl')}/auth/user/`, { body: { id: 'nonOwnerId', roles: [{name: ROLE_VENUE_MANAGER}] } }).as('getNonOwnerUser');

        cy.visit(`/venues/${this.ownedVenueId}/edit`);
        cy.wait('@getNonOwnerUser');
        cy.wait('@getVenueDetails'); // Ensure venue details are loaded to check ownership

        cy.contains('You are not authorized to edit this venue.').should('be.visible');
        cy.get('h1').contains(`Edit Venue: ${mockVenueData.name}`).should('not.exist');
    });
    ...
    context('When logged in as Venue Manager (Owner)', function() {
        beforeEach(function() {
            ...
            cy.intercept('GET', `${Cypress.env('apiUrl')}/auth/user/`, { body: { id: mockVenueData.owner.id, roles: [{name: ROLE_VENUE_MANAGER}] } }).as('getOwnerUser');
            cy.visit(`/venues/${this.ownedVenueId}/edit`);
        ...
        it('allows owner VENUE_MANAGER to see the edit page with pre-filled form', function() {
            ...
            cy.get('textarea[name="address"]').should('have.value', mockVenueData.address);
        ...
        it('allows owner VENUE_MANAGER to update the venue', function() {
            ...
            cy.intercept('PATCH', `${Cypress.env('apiUrl')}/venues/${this.ownedVenueId}/`).as('updateVenueApi');
            ...
            cy.contains(`Venue "${updatedName}" updated successfully!`).should('be.visible');
