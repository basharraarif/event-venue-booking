...
describe('Event Creation Page (/dashboard/organizer/events/create)', () => {
    ...
    const adminUser = { // Admin user for creating venues needed by event form
    ...
    before(() => {
        ...
        cy.request({
            ...
            body: { username: adminUser.username, email: adminUser.email, password: adminUser.password, password2: adminUser.password },
            failOnStatusCode: false // Allow failure if user already exists from previous partial run
        }).then((regResp) => {
            // Log in as admin to get token
            cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: adminUser.email, password: adminUser.password } })
            .then(loginResp => {
                ...
                cy.request({
                    ...
                    body: { name: `Test Venue for Event Create ${uniqueId}`, address: '123 Test St', capacity: 100 }
                }).then(venueResp => {
        ...
        cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/registration/`, body: { username: eventOrganizerUser.username, email: eventOrganizerUser.email, password: eventOrganizerUser.password, password2: eventOrganizerUser.password } })
            .then(() => cy.request({ method: 'POST', url: `${Cypress.env('apiUrl')}/auth/login/`, body: { email: eventOrganizerUser.email, password: eventOrganizerUser.password } }))
            .then(loginResp => {
                ...
                cy.log(`Event Organizer token obtained. Role assignment might be needed: ${eventOrganizerUser.email}`);
            });
    ...
    context('When logged in as Event Organizer', () => {
        beforeEach(function() {
            ...
            cy.intercept('GET', `${Cypress.env('apiUrl')}/auth/user/`, {
                statusCode: 200,
                body: { pk: 'eoUserId', id: 'eoUserId', username: eventOrganizerUser.username, email: eventOrganizerUser.email, roles: [{ name: ROLE_EVENT_ORGANIZER }] },
            }).as('getUserEO');
            // Mock calls for EventForm data if it makes them (e.g., categories, venues)
            cy.intercept('GET', `${Cypress.env('apiUrl')}/venues/?limit=1000&offset=0`, { statusCode: 200, body: { results: [{id: this.testVenueId, name: 'Test Venue for Event Create'}]} }).as('getVenuesForForm');
            cy.intercept('GET', `${Cypress.env('apiUrl')}/events-management/categories/`, { statusCode: 200, body: [{id: 'cat1', name: 'Test Category'}] }).as('getCategoriesForForm');
        ...
        it('allows EVENT_ORGANIZER to fill and submit the event form successfully', function() {
            ...
            cy.intercept('POST', `${Cypress.env('apiUrl')}/events-management/events/`).as('createEventApi');

            cy.get('input[name="name"]').type(eventName);
            cy.get('textarea[name="description"]').type('An exciting E2E test event.');
            ...
            const endTime = new Date(Date.now() + 3 * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000); // 2 hours after start
            cy.get('input[name="start_time"]').type(startTime.toISOString().substring(0, 16));
            cy.get('input[name="end_time"]').type(endTime.toISOString().substring(0, 16));
