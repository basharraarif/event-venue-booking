...
describe('Event Booking and Payment Flow E2E Test', () => {
            ...
            cy.request({
                ...
                cy.request({
                    ...
                    cy.request({
                        ...
                        headers: { Authorization: `Token ${authToken}` },
                        body: { name: 'E2E Test Venue', address: '123 E2E St', capacity: 100, owner: testUserId } // Assign owner
                    }).then(venueResp => {
                        ...
                        cy.request({
                            ...
                            body: {
                                ...
                                description: "A paid event for E2E testing", organizer: testUserId // Assign organizer
                            }
                        ...
                        cy.request({
                            ...
                            body: {
                                ...
                                description: "A free event for E2E testing", organizer: testUserId // Assign organizer
                            }
    ...
    context('Paid Event Booking with Stripe Interaction', () => {
        ...
        const typeInStripeInput = (inputSelector: string, value: string) => {
            ...
            cy.get('iframe[title="Secure payment input frame"]').its('0.contentDocument.body').should('not.be.empty')
              .then(cy.wrap)
              .find(inputSelector)
        ...
        const completeStripeForm = () => {
            ...
            cy.get('iframe[src*="stripe.com"]').its('0.contentDocument.body').should('not.be.empty')
              .then(cy.wrap)
              .find('input[name="cardNumber"], input#Field-cardNumberInput, [data-elements-stable-field-name="cardNumber"]') // Common selectors
              .type('4242')
              .type('4242')
              .type('4242')
              .type('4242');
            ...
            cy.get('iframe[src*="stripe.com"]').its('0.contentDocument.body').should('not.be.empty')
              .then(cy.wrap)
              .find('input[name="cardExpiry"], input#Field-cardExpiryInput, [data-elements-stable-field-name="cardExpiry"]')
              .type('12/29'); // Future date

            cy.get('iframe[src*="stripe.com"]').its('0.contentDocument.body').should('not.be.empty')
              .then(cy.wrap)
              .find('input[name="cardCvc"], input#Field-cardCvcInput, [data-elements-stable-field-name="cardCvc"]')
              .type('123');

            cy.get('iframe[src*="stripe.com"]').its('0.contentDocument.body').should('not.be.empty')
              .then(cy.wrap)
              .find('input[name="postalCode"], input#Field-postalCodeInput, [data-elements-stable-field-name="postalCode"]')
              .type('90210');
        ...
        it('successfully books and simulates a successful payment', function() {
            ...
            cy.intercept('POST', `${Cypress.env('apiUrl')}/bookings/`, {
                statusCode: 201,
                body: { id: 'mockBookingSuccess123', status: 'pending_payment', event: this[paidEventAlias], number_of_tickets: 1, total_price: "10.00" },
            ...
            cy.window().then((win) => {
                if (win.stripe) { // Ensure stripe object is available
                    cy.stub(win.stripe, 'confirmCardPayment').resolves({
            ...
            cy.get('button[type="submit"]').contains(/pay|confirm order/i).click();
            ...
            cy.contains('h1', /payment successful/i, { timeout: 10000 }).should('be.visible');
            cy.contains('Your booking is confirmed.', { timeout: 10000 }).should('be.visible');
            ...
            cy.get('@authToken').then(token => {
                cy.request({
                    ...
                    cy.log('Frontend shows success. Backend status depends on actual webhook or further frontend calls.');
                });
            });
        });
        ...
        it('handles a simulated failed payment', function() {
            ...
            cy.intercept('POST', `${Cypress.env('apiUrl')}/bookings/`, {
                statusCode: 201,
                body: { id: 'mockBookingFail123', status: 'pending_payment', event: this[paidEventAlias], number_of_tickets: 1, total_price: "10.00" },
            ...
            cy.window().then((win) => {
                if (win.stripe) {
                    cy.stub(win.stripe, 'confirmCardPayment').resolves({
                     ...
                     throw new Error("Stripe.js not found on window.stripe for failure mock");
                }
            });
            ...
            cy.contains('Your card was declined by the bank.', { timeout: 10000 }).should('be.visible');
            // Verify user is still on the checkout page
            cy.url().should('include', '/checkout/mockBookingFail123');
            // Verify form might be re-enabled or a "try again" option is available
            cy.get('button[type="submit"]').contains(/pay|confirm order/i).should('not.be.disabled');
        });
    });

    it('successfully books a free event', function() { // Copied from original, seems okay
        cy.visit(`/events/${this[freeEventAlias]}`);
        cy.contains('h2', 'E2E Free Event').should('be.visible');
        cy.get('input[name="numberOfTickets"]').clear().type('1');
        cy.intercept('POST', `${Cypress.env('apiUrl')}/bookings/`, {
            statusCode: 201,
            body: { id: 'mockBookingFree123', status: 'confirmed', event: this[freeEventAlias], number_of_tickets: 1, total_price: "0.00"},
