    ...
    before(() => {
        ...
        cy.request({
            ...
            cy.request({
                ...
                cy.log(`Venue Manager token obtained. Manual role assignment might be needed if not default: ${venueManagerUser.email}`);
            });
        });
    ...
    context('When logged in as Venue Manager', () => {
        beforeEach(function() { // Use function() to access alias context
            ...
            cy.intercept('GET', `${Cypress.env('apiUrl')}/auth/user/`, {
                statusCode: 200,
                body: {
                    pk: 1, id: 1, username: venueManagerUser.username, email: venueManagerUser.email,
        ...
        it('allows VENUE_MANAGER to see the "Add New Venue" page and form', () => {
            ...
            cy.get('button[type="submit"]').contains('Create Venue').should('be.visible');
        });

        it('allows VENUE_MANAGER to fill and submit the venue form successfully', () => {
            const venueName = `E2E Test Venue ${new Date().getTime()}`;
            cy.intercept('POST', `${Cypress.env('apiUrl')}/venues/`).as('createVenueApi');
            ...
            cy.contains(`Venue "${venueName}" created successfully!`).should('be.visible');
