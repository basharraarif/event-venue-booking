...
describe('Role-Based UI and Permissions E2E Tests', () => {
    ...
    cy.then(() => {
      ...
      cy.request({
        ...
        body: {
          ...
          address: '123 VM St',
          capacity: 100,
          owner: users.venueManager.id,
        },
      }).then((resp) => {
        venueByVMId = resp.body.id;
      });
      ...
      cy.request({
        ...
        cy.request({
          ...
          headers: { Authorization: `Token ${users.admin.token}` },
          body: {
            name: 'Admin General Event',
            venue: venueByAdminId,
            ticket_price: '30.00',
            organizer: users.admin.id,
            start_time: new Date(
  ...
  context('Event Organizer UI and Permissions', () => {
    ...
    it('should see UI for managing own events but not venues', () => {
      ...
      cy.get('nav a[href="/dashboard/organizer/events/new"]').should(
        'be.visible'
  ...
  context('Venue Manager UI and Permissions', () => {
    ...
    it('should see UI for managing own venues but not events', () => {
      ...
      cy.get('nav a[href="/dashboard/manager/venues/new"]').should(
        'be.visible'
      ); // Link to create new venue
      cy.get('nav a[href="/dashboard/organizer/events/new"]').should(
        'not.exist'
      ); // No link to create new event
